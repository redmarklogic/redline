...
**GitHub backlog/board operations:** Any create, update, move, sprint, or field change on the Redline Projects board → load `github-projects` skill **first**. Field-schema mutations (`updateProjectV2Field` on single-select or iteration fields) are destructive — the skill's snapshot-and-restore procedure is binding.
