...
Run after all files are indexed and renamed:
    ...
    c:\\Users\\harel\\Documents\\products\\redline\\.venv\\Scripts\\python.exe .agents\\tools\\library\\dedup_index.py
...
for h, rows in hash_to_rows.items():
    if len(rows) > 1:
        primary_path = rows[0][7].value
        for dup_row in rows[1:]:
            existing = dup_row[12].value or ""
            if "DUPLICATE of" not in existing:
                dup_row[12].value = f"DUPLICATE of {primary_path}; {existing}".strip("; ")
                print(f"Flagged: {dup_row[8].value}")
                flagged += 1
