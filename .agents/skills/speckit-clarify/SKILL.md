...
1. Run `.specify/scripts/powershell/check-prerequisites.ps1 -Json -PathsOnly` from repo root **once** (combined `--json --paths-only` mode / `-Json -PathsOnly`). Parse minimal JSON payload fields:
   ...
   - If JSON parsing fails, abort and instruct user to verify feature branch environment or re-run the spec generation step.
...
- If spec file missing, instruct user to generate the spec first (do not create a new spec here).
