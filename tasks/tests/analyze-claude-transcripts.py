...
def main():
    ...
    sub_files = [
        f
        for f in proj_dir.glob("*/subagents/*.jsonl")
        if datetime.fromtimestamp(f.stat().st_mtime, tz=UTC) > cutoff
    ]
        ...
        with f.open(encoding="utf-8", errors="replace") as fh:
            for line in fh:
                try:
                    ...
                    continue
                ts = (
                    parse_ts(rec.get("timestamp", "")) if rec.get("timestamp") else None
                )
                ...
                if rec.get("type") == "assistant":
                    ...
                    for blk in msg.get("content") or []:
                        if isinstance(blk, dict) and blk.get("type") == "tool_result":
                            ...
                            size = (
                                len(json.dumps(content, default=str)) if content else 0
                            )
    ...
    print(
        f"=== {sessions} main sessions, {len(sub_files)} subagent transcripts, last {DAYS} days ==="
    )
