...
- [ ] T001 Create directories `infra/bootstrap/` and `infra/terraform/`; create empty
...
- [ ] T002 Populate `infra/terraform/terraform.tfvars` using the schema from
...
- [ ] T006 [US1] Write `infra/terraform/versions.tf` (or add to `main.tf`) declaring
  the required Terraform version (`>= 1.6`) and `hashicorp/google` provider version
  (`~> 5.0`); pin to specific minor version
- [ ] T007 [US1] Implement `infra/bootstrap/bootstrap.sh`:
...
- [ ] T009 [P] [US1] Implement `infra/terraform/backend.tf`: configure GCS remote
  backend pointing at `state_bucket` value from `terraform.tfvars`
- [ ] T010 [P] [US1] Implement `infra/terraform/variables.tf`: declare all variables
  matching the schema in `contracts/terraform-variables.md`
- [ ] T011 [P] [US1] Implement `infra/terraform/main.tf`: configure `google` provider
  ...
  in bootstrap)
- [ ] T012 [US1] Implement `infra/terraform/apis.tf`: one `google_project_service`
  resource per entry in `var.apis` using `for_each`; set `disable_on_destroy = false`
- [ ] T013 [US1] Implement `infra/terraform/billing.tf`: `google_billing_project_info`
  resource linking `var.billing_account` to the project; add idempotency note (resource
  is safe to re-apply)
- [ ] T014 [US1] Implement `infra/terraform/outputs.tf`: output `project_number`
...
- [ ] T020 [US2] Add a `README.md` to `infra/` documenting:
...
- [ ] T021 [US2] Cross-reference `infra/terraform/terraform.tfvars` from the project's
...
- [ ] T022 [US2] Add `specs/005-gcp-project-baseline/verify-output.txt` to `.gitignore`
...
- [ ] T023 [P] Add SSOT comment at top of `infra/terraform/terraform.tfvars`
  referencing ADR-001 and ADR-020; add ADR-020 reference to `infra/bootstrap/bootstrap.sh`
  header
- [ ] T024 Update `specs/005-gcp-project-baseline/checklists/requirements.md` to
