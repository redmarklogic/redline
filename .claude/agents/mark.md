...
| Plan the next sprint: set sprint goal, select tasks from backlog, sequence by dependency and risk, name out-of-scope items, write plan to docs/product/tasks/, update board. Trigger phrases: "sprint planning", "plan the sprint", "plan sprint N", "what goes in the next sprint", "start sprint planning". | `agile-sprint-planning` |
| Audit and declutter the backlog: classify every backlog item against completed work, ADRs, and active bets; produce a per-row decision table; execute only founder-approved rows (verbal approval per row, in session). **On-demand only — founder-triggered, never self-scheduled.** Trigger phrases: "groom the backlog", "backlog audit", "run a grooming pass", "declutter the board". | `agile-backlog-grooming` |
