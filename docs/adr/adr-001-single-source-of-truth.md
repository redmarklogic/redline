...
| **Skill layer assignment (L0-L9)** | `docs/architecture/skills-architecture.md` (Layer Definitions section) | Layer assignments stated in any other document (`skills-system.md`, `AGENTS.md`, `docs/people/skills-taxonomy.md`, inline in skill files) | ADR-009 |
| **Skill layer field in machine-readable registry** | `skills-lock.json` (`layer` field, derived from `skills-architecture.md` via `hooks/sync-layer-to-lock.py`) | Setting `layer` in the lock file without first updating `skills-architecture.md`; `layer` diverging from the architecture doc | ADR-009 |
...
| **Skills narrative and onboarding** | `docs/knowledge/software-engineering/skills-system.md` (examples, analogies, worked flows — no skill enumeration by layer) | Enumerating skills by layer or tier in the narrative doc; re-stating layer assignments outside `docs/architecture/skills-architecture.md` |  |
| **Skill domain category (vocabulary + per-skill assignment)** | `docs/people/skills-taxonomy.md` (Category Vocabulary section + catalog) | Parallel grouping schemes in other documents (e.g. group columns in `skills-architecture.md`); categories used without a registered definition |  |
| **Skill purpose / use-when trigger** | `SKILL.md` frontmatter `description` of each skill | Hand-written purpose text treated as authoritative; catalog *Use when* column diverging from frontmatter (it is a derived cache) |  |
| **Skill coverage gaps and lifecycle ledger** | `docs/people/skills-gaps.md` | Gap, pending, or deprecation state embedded in catalog rows or the architecture doc; gaps tracked only in chat or sync reports |  |
| **Agent–skill topology visuals** | Generated from `skills-lock.json` during `hr-sync-agent-topology` runs; embedded dated in that sync's report | Hand-maintained living topology diagrams in any document |  |
